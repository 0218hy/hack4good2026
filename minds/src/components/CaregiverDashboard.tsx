...
export default function CaregiverDashboard({ activities, setActivities }: CaregiverDashboardProps) {
  const { t } = useLanguage();
  ...
  // Set list as default view when on care schedule tab
  useEffect(() => {
  ...
  const handleConfirmSignup = (caregiverData: any) => {
    if (selectedActivity) {
      // Increment participant count only (caregivers sign up participants)
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {view === 'calendar' ? (
          <CalendarView
            ...
            userRole="caregiver"
            showLegend={activeTab !== 'care-schedule'}
          />
        ) : (
          <ListView
            activities={activitiesToShow}
      ...
      {showManageNeedsModal && (
        <div className="fixed inset-0 backdrop-blur-sm bg-white/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">Manage Care Receiver Needs 管理照顾对象需求</h2>
            <div className="space-y-5">
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                />
                <span className="text-lg text-gray-700">Wheelchair-bound 需要轮椅</span>
              </label>
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  ...
                  className="w-5 h-5 text-sky-500 border-2 border-gray-300 rounded focus:ring-sky-500"
                />
                <span className="text-lg text-gray-700">Sign language support 手语支持</span>
              </label>
              <div>
                <label className="block text-base font-bold text-gray-700 mb-3">Other Needs 其他需求</label>
                <textarea
