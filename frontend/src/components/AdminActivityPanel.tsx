...
export default function AdminActivityPanel({ activity, isOpen, onClose, onDeleteActivity, onEditActivity }: AdminActivityPanelProps) {
  ...
  const mockParticipantSignups = [
    { id: '1', name: 'Alice Johnson', email: 'alice@example.com', attended: false, paymentStatus: 'unpaid' as const },
    { id: '2', name: 'Bob Smith', email: 'bob@example.com', attended: true, paymentStatus: 'paid' as const },
    { id: '3', name: 'Carol White', email: 'carol@example.com', attended: false, paymentStatus: 'paid' as const },
  ];
  ...
  const downloadCSV = (type: 'participants' | 'volunteers' | 'combined') => {
    ...
    if (type === 'participants') {
      csvContent = 'Name,Email,Attended,Payment Status\n';
      participants.forEach(p => {
        csvContent += `${p.name},${p.email},${p.attended ? 'Yes' : 'No'},${p.paymentStatus}\n`;
      });
      ...
      csvContent = 'Type,Name,Email,Caregiver Name,Caregiver Email,Attended,Payment Status\n';
      participants.forEach(p => {
        const caregiver = mockCaregiverInfo.find(c => c.participantId === p.id);
        csvContent += `Participant,${p.name},${p.email},${caregiver?.caregiverName || 'N/A'},${caregiver?.caregiverEmail || 'N/A'},${p.attended ? 'Yes' : 'No'},${p.paymentStatus}\n`;
      });
      volunteers.forEach(v => {
        csvContent += `Volunteer,${v.name},${v.email},N/A,N/A,${v.attended ? 'Yes' : 'No'},N/A\n`;
      });
    }
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-8">
            <div className="bg-sky-50 rounded-lg p-4">
              ...
              <p className="text-2xl font-bold text-sky-600">
                {participants.length} / {activity.participantCapacity}
              </p>
              <p className="text-sm text-gray-600 mt-1">
                {activity.participantVacancy} spots remaining
              </p>
            </div>
            <div className="bg-emerald-50 rounded-lg p-4">
              <h3 className="font-semibold text-gray-900 mb-2">Volunteers</h3>
              <p className="text-2xl font-bold text-emerald-600">
                {volunteers.length} / {activity.volunteerCapacity}
              </p>
              <p className="text-sm text-gray-600 mt-1">
                {activity.volunteerVacancy} spots remaining
              </p>
            </div>
          </div>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">Delete Activity?</h2>
